...
## 6. UML class diagram (entities)

The same model expressed as a UML class diagram, mapping each TypeORM entity to a class
(TypeScript field names & types). Relationships use UML notation: **composition** (filled
diamond) where the parent owns the lifecycle (`CASCADE`), **directed association** for a
plain reference (`RESTRICT`), and **dependency** (dashed) for logical / cross-service
links that have no physical foreign key.
...
classDiagram
    ...
    class ReservationEntity {
        ...
        +Date expiresAt
        +ReservationSeatEntity[] reservationSeats
        +Date createdAt
        +Date updatedAt
    }
    ...
    ReservationEntity "1" *-- "0..*" ReservationSeatEntity : owns (CASCADE)
    ReservationSeatEntity "0..*" --> "1" SeatEntity : holds (RESTRICT)
    SeatEntity ..> SeatStatus : status
    ReservationEntity ..> ReservationStatus : status

    %% ---- logical / cross-service (no DB FK) ----
    RefreshTokenEntity ..> UserEntity : userId (logical)
    RefreshTokenEntity ..> RefreshTokenEntity : replacedBy (rotation)
    ReservationEntity ..> UserEntity : userId (cross-service)
    IdempotencyKeyEntity ..> UserEntity : userId (cross-service)
    IdempotencyKeyEntity ..> ReservationEntity : reservationId (nullable)
```

> UML legend: `*--` composition (lifecycle ownership) · `-->` directed association
> (reference only) · `..>` dependency (logical link, **no DB FK**) · `«PK»`/`«FK»`/`«unique»`
> are stereotypes · `«enumeration»` marks the shared SDK enums.

---

## 7. UML state machines (status enums)

How the two status enums transition at runtime — a UML state-machine view of the lifecycle
each enum drives.
...
```mermaid
stateDiagram-v2
    [*] --> PENDING : create (seats held, expiresAt set)
    PENDING --> CONFIRMED : pay / confirm
    PENDING --> EXPIRED : expiresAt passes (expiry sweep)
...
### Seat lifecycle (`SeatStatus`)

```mermaid
stateDiagram-v2
    [*] --> AVAILABLE
    AVAILABLE --> RESERVED : added to a PENDING reservation (is_active hold)
    RESERVED --> BOOKED : reservation CONFIRMED
    RESERVED --> AVAILABLE : reservation EXPIRED / CANCELLED (hold released)
    BOOKED --> [*] : seat sold
```

> Seat status mirrors the active `reservation_seats` hold: a seat is `RESERVED` while a
> `PENDING` reservation holds it (`is_active = true`), becomes `BOOKED` on confirmation, and
> returns to `AVAILABLE` when the hold is released on expiry/cancellation.
