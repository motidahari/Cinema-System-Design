...
const EMAIL = 'lockout@cinema.test';
const IP = '10.0.0.1';
const THRESHOLD = 3;
const WINDOW_MIN = 15;
...
describe('LoginAttemptDao (integration)', () => {
    ...
    afterEach(async () => {
        await dataSource.query(`DELETE FROM identity.login_attempts`);
    });
    ...
    describe('recordFailure, Given:Different (email, ip) pairs, When:Recording', () => {
        it('should track counters independently', async () => {
            ...
            expect(await dao.isLocked('other@cinema.test', IP)).toBe(false);
        });
    });
    ...
    describe('clear, Given:No existing row, When:Clearing', () => {
        it('should complete without error', async () => {
            await expect(dao.clear('nobody@cinema.test', IP)).resolves.toBeUndefined();
        });
    });
});
